...
pub fn render_line(e: &NetEvent) -> String {
    match &e.transport {
        Transport::Udp { src_port, dst_port, length, is_dns } => {
        ...
        Transport::Tcp { src_port, dst_port, length } => match e.direction {
            Direction::Inbound => format!(
                "[{}]: {}:{} {}<==== [TCP] ====={} {}:{}; length: {}",
        ...
        Transport::Arp { operation, sender_mac, sender_ip, target_mac, target_ip } => match e.direction {
            Direction::Inbound => format!(
                "[{}]: {}({}) {}<==== [ARP] ======{} {}({}); operation: {:?}",
...
pub fn print_event(e: &NetEvent) { println!("{}", render_line(e)); }
...
mod tests {
    ...
    fn render_udp_dns_inbound_no_color() {
        ...
        let e = NetEvent::new(
            ...
            Transport::Udp { src_port: 53, dst_port: 53000, length: 42, is_dns: true },
        );
        let line = render_line(&e);
        assert_eq!(line, "[eth0]: 10.0.0.2:53000 <===== [UDP/DNS] ===== 1.1.1.1:53; length: 42");
    }
    ...
    fn render_tcp_outbound_no_color() {
        ...
        let e = NetEvent::new(
            ...
            Transport::Tcp { src_port: 50123, dst_port: 443, length: 1200 },
        );
        let line = render_line(&e);
        assert_eq!(line, "[eth0]: 10.0.0.2:50123 ===== [TCP] =====> 93.184.216.34:443; length: 1200");
    }
    ...
    fn render_icmp_echo_reply_inbound_no_color() {
        ...
        let line = render_line(&e);
        assert_eq!(line, "[eth0]: 10.0.0.2 <==== [ICMP echo reply] ===== 8.8.8.8 (seq=1, id=99)");
    }
